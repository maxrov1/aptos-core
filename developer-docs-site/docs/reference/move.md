...
| Function calling | Explicit static dispatch | Static dispatch | Dynamic dispatch |
...
In Move, a module can define how resources can be access and their contents modified regardless of the presence of the account owner's signer. This means that a programmer could accidentally create a resource that allows other users to arbitrarily insert or remove assets from another users account.

In our development, we have several examples of where we have allowed permission to access and where we have prevented it:

* A Token cannot be directly inserted into another users account unless they already have some of that Token
...
* In Coin a user can directly transfer into another users account so long as they have a resource for storing that coin
